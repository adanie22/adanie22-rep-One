...
def create_xls_compute(l,site):
    #orig: wb2 = xlwt.Workbook(encoding='utf-8')
    wb2 = xlsxwriter.Workbook(encoding='utf-8')
    ...
    #orig: sht = wb2.add_sheet('index')
    sht = wb2.add_worksheet('index')

    #hdr = [i for i in l[0].keys() if i in ('hostname','id','image_id','instanceId','name','parent','privateAddress','stack','state')]
    #hdr = [('hostname',10000),('id',5000),('image_id',10000),('instanceId',10000),('name',10000),('parent',10000),('privateAddress',10000),('stack',10000),('state',10000)]
    hdr = [['hostname',10000],['id',5000],['image_id',10000],['instanceId',10000],['name',10000],['parent',10000],['privateAddress',10000],['stack',10000],['state',10000]]    
    #pp(hdr)
    #pp(len(hdr))
    #pp(range(0,len(hdr)))
    ...
    for d in l:
        for m,n in enumerate(hdr):
            ...
            print(d)
            if n == 'stack':
                sht.write(nrow,m,d[n]['name'])
                #if m >= 2:
                    #sht.write(nrow,m-2,d['stack']['name'])
            else:
                sht.write(nrow,m,d[n][0])
                #if m >= 2:
                    #sht.write(nrow,m-2,d['stack'])
        nrow += 1
    """
            
    print "Saving reports %s",(fp)
    sht.autofilter('A1:J200')
    wb2.save(fp)
    return fp
