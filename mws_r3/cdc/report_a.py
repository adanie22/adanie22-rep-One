...
def list_compute(asset_type,env=default.ENV,hdrs={'accept':'application/json'}):
    ...
    if r.status_code == 200:
        ...
        for d in r.json()['links']:
            ...
            j2 = r.json()
            d2 = {}
            [set_key(d2,j2,m) for m in ('id','name','parent','instanceId','privateAddress','hostname','image_id','stack','name','state')]
            """
            set_key(d2,j2,'name')
            set_key(d2,j2,'parent')
            set_key(d2,j2,'instanceId')
            set_key(d2,j2,'privateAddress')
            set_key(d2,j2,'hostname')
...
def create_xls_compute(l):
    ...
    fp = os.path.join(dp_root,'cdc-compute-%s.xls'%ts)
...
def create_xls(l_wkld_pkg_pvars,d_pkgs,d_scrpts,bps_cfg):
    """
    ...
    def dump_wkld_resource_details(sht,nrow,bp,wkld,wkld_pvars,l_pkg_items,compnt_id,style,style_yellow):
        ...
        if len(wkld.ro.resources.items()) == 0:
            ...
            for k,v in wkld.ro.resources.iteritems():
                ...
                write_row(sht,nrow,(bp.short_id,compnt_id,bp.parent_name,bp.name,wkld.name,resource_type,resource_name,resource_value),style)
                nrow += 1
        return nrow
    ...
    def create_resource_sht(wb2,style,l_wkld_pkg_pvars,d_pkgs,d_scrpts):
        hdr = [('bp_id',2500),('component',3000),('bp_container',4000),('bp_name',10000),('wkld_name',6000),('resource type',5500),('resource name',7000),('resource value',3000)]
    ...
    fp = os.path.join(dp_root,'cdc-blueprints-%s.xls'%ts)
...
#        scrpt_rpt_sht.write(nrow,ncol,'\n'.join(scrpt_vars.keys()),style)
    ...
    dp_rpt = os.path.join(dp_root,'cdc-baseline.xls')
    """ 
