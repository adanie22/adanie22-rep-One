...
function UpdateInputFile ([string] $inputFile)
{
    ...
    {
        $xmlinput.Configuration.Farm.Services.OutgoingEmail.SMTPServer = (Get-VariableValue $SMTP_SERVER "smtp" $useHardcodedDefaults) + ".$domainFull"
    }
    else
    {
        $xmlinput.Configuration.Farm.Services.OutgoingEmail.SMTPServer = (Get-VariableValue $SMTP_SERVER "smtp" $useHardcodedDefaults)
    }

    $xmlinput.Configuration.Farm.Services.OutgoingEmail.EmailAddress = (Get-VariableValue $EMAIL_ADDRESS "mws2Admin") + "@$domainFull"
    $xmlinput.Configuration.Farm.Services.OutgoingEmail.ReplyToEmail = (Get-VariableValue $REPLY_TO_EMAIL "replyTo" $useHardcodedDefaults) + "@$domainFull"
    ...
    $xmlinput.Configuration.ServiceApps.EnterpriseSearchService.ContactEmail = (Get-VariableValue $ENTSEARCH_CONTACTEMAIL "mws2SearchAdmin" $useHardcodedDefaults) + "@$domainFull"
    ...
    $searchServiceApp.SearchCenterUrl = (ConstructURL $searchServiceApp.SearchCenterUrl "$domainFull/pages" $useSSL)
    ...
    $portalWebAppUrl = (ConstructURL $portalServer $domainFull $useSSL)
    $portalWebApp.url = $portalWebAppUrl
    $portalWebAppPort = (Get-VariableValue $PORTAL_WEBAPP_PORT "80" $useHardcodedDefaults)
    ...
    foreach($siteCol in $portalWebApp.SiteCollections.SiteCollection)
    {
        ...
        {
            $siteCol.siteUrl = (ConstructURL $siteCol.siteUrl $domainFull  $useSSL)
        }

        $siteCol.SearchUrl = (ConstructURL $siteCol.SearchUrl "$domainFull/pages" $useSSL)
    ...
    if ($myWebApp -ne $null)
    {
        ...
        $myWebAppUrl = (ConstructURL (Get-VariableValue $MYSITES_WEBAPP_URL "my" $useHardcodedDefaults) $domainFull $useSSL) #(Get-VariableValue $MYSITES_WEBAPP_URL "my" $useHardcodedDefaults)
        $myWebApp.url = $myWebAppUrl
        $myWebAppPort = (Get-VariableValue $MYSITES_WEBAPP_PORT "80" $useHardcodedDefaults)
        ...
        $rootSiteColMySites.SearchUrl = (ConstructURL $rootSiteColMySites.SearchUrl "$domainFull/pages" $useSSL)
    ...
    $xmlinput.Configuration.ServiceApps.UserProfileServiceApp.MySiteHostLocation = (ConstructURL $mySitesCol $domainFull  $useSSL)
    ...
    $xmlinput.Configuration.ServiceApps.AppManagementService.AppDomain = (Get-VariableValue $APPMGMT_DOMAIN "app" $useHardcodedDefaults) + ".$domainFull"
...
$parameters.Add("ADMIN", $user)
$parameters.Add("ADMIN_PWD", $password)
