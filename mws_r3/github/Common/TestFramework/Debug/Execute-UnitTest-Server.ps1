...
try
{
    ...
    $testFolder = ([string](Get-VariableValue $TESTFRAMEWORK "TestFramework" $true))
    $scripts = ([string](Get-VariableValue $SCRIPTS "UnitTest-Server-Common.ps1" $true)).Split(";")
    ...
    foreach($script in $scripts)
    {
        ...
        # DEBUG
        . .\$testFolder\UnitTest-Server-SharePoint.ps1 -scriptPath $scriptPath -testFolder $testFolder
