...
if ((CheckFileExists($MWSREGISTRYXMLFILENAME)) -ne $true)
{
    $error = "ERROR: $MWSREGISTRYXMLFILENAME does not exist"  
    log $error
    throw $error
}
...
$argument = "-file $scriptPath\ConfigureWindowsCluster.ps1 -scriptPath $scriptPath -clusterXMLfile $SHAREDWINDOWSCLUSTERXMLFILENAME -registryXMLfile $MWSREGISTRYXMLFILENAME -currentUser '$domain\$currentUser' ; exit `$LastExitCode"
