...
{  
    try
    {
        ...
        foreach($clusterNetwork in $clusterNetworks)
        {
            ...
            $networkRole = $clusterNetwork.Role
            if($networkRole -eq '0')
            {
                $networkDesription = "0 - Do NOT allow cluster or client network communications."
            }
            elseif($networkRole -eq '3')
            {
                $networkDesription = "3 - Allow cluster and client network communications."
            }
            else
            {
                $networkDesription = $networkRole
            }

            $object | Add-Member -Name "Cluster Network$($networkCount)" -MemberType Noteproperty -Value "$($clusterNetwork.Name) ($($isClusterNetworkRunning)) ($($networkDesription))"
        }
