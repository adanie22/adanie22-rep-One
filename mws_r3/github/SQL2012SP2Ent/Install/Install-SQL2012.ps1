...
{
    $isWindowsClusterConfigured = GetNodeValue $registryXMLfile 'IsWindowsClusterConfigured' 
    
    if($isWindowsClusterConfigured -eq 'False')
    {
        # as all of the install/config of shared disks and windows cluster occurs on the primary node
        # this check is here to ensure that the secondary node will be able to install the
        # SQL cluster. If ANY windows cluster is found then it is assumed to be the correct one
        $clusterExists = Get-Cluster
        if($clusterExists -ne $null) # then cluster exists
        {
            SetNodeValue $registryXMLfile 'IsSharedDiskConfigured' 'True'
            SetNodeValue $registryXMLfile 'IsWindowsClusterConfigured' 'True'
            $isWindowsClusterConfigured = 'True'
        }
    }

    if($isWindowsClusterConfigured -eq 'True')
    {
        log "INFO: Pre-requisistes completed. Windows Cluster is available."
        $isSQLClusterConfigured = GetNodeValue $registryXMLfile 'IsSQLClusterConfigured' 
        if($isSQLClusterConfigured -eq 'True')
        {
            log "ERROR: SQL Cluster has already been installed and will NOT be reinstalled."
            return $false
        }
        else
        {
    ...
    {
        log "ERROR: Pre-requisistes NOT completed. Windows Cluster is NOT available"
...
$registryXMLfile = "$MWSREGISTRYXMLFILENAME"

$preReqsCompleted = CheckPreReqs 'IsWindowsClusterConfigured'
if(!$preReqsCompleted)
{
    return
}
...
SetNodeValue $registryXMLfile 'IsSQLClusterConfigured' 'True'
