...
function CreateADObjects([string]$clusterObjects)
{
    ...
    foreach($cluster in $clusters)
    {
        ...
        $OU = "OU=Database,OU=Servers,OU=Customer,DC=$($domainSplit[0]),DC=$($domainSplit[1])"
        log "******************"
        ...
        catch
        {
            ...
            $parentOU = "OU=Servers,OU=Customer,DC=$($domainSplit[0]),DC=$($domainSplit[1])"
...
log "INFO: Created Cluster AD Objects for '$clusterNames'"    
