...
if ((CheckFileExists($SPNXMLFILE)) -ne $true)
{
    $error = "ERROR: SPNXMLFILE does not exist"  
    log $error
    throw $error
}
...
$argument = "-file $scriptPath\install\Configure-SPNs.ps1 -scriptPath $scriptPath -SPNXMLFILE $SPNXMLFILE ; exit `$LastExitCode"
