...
function UpdateInputFile ([string] $inputFile, [string] $password)
{
    ...
    $xmlinput.Configuration.CertificateName = Get-VariableValue $CERTIFICATE_NAME “MWS2R2 OfficeWebApps” $useHardcodedDefaults

    log "INFO: Setting ExternalURL"
    $extErl = Get-VariableValue $EXTERNAL_URL "" $useHardcodedDefaults
    $xmlinput.Configuration.ExternalURL = (ConstructURL $extErl $domainFull $useSSL)
    
    log "INFO: Setting InternalURL"
    $intErl = Get-VariableValue $INTERNAL_URL "OfficeApps" $useHardcodedDefaults
    $xmlinput.Configuration.InternalURL =  (ConstructURL $intErl $domainFull $useSSL)
