    ...
    if ($joinToFarm -eq $true)
    {
        if ($PrimaryServer -eq $currentServer)
        {
            ...
            if ($useSSL -eq $true)
            {
                ...
                {
                    log "New-OfficeWebAppsFarm -CacheLocation $CacheLocation -CacheSizeInGB $CacheSizeInGB -CertificateName `"$CertificateName`" -InternalURL $InternalURL -LogLocation $LogLocation -LogRetentionInDays $LogRetentionInDays -MaxMemoryCacheSizeInMB $MaxMemoryCacheSizeInMB -RenderingLocalCacheLocation $RenderingLocalCacheLocation -EditingEnabled:true"
                    New-OfficeWebAppsFarm -CacheLocation $CacheLocation -CacheSizeInGB $CacheSizeInGB -CertificateName $CertificateName -InternalURL $InternalURL -LogLocation $LogLocation -LogRetentionInDays $LogRetentionInDays -MaxMemoryCacheSizeInMB $MaxMemoryCacheSizeInMB -RenderingLocalCacheLocation $RenderingLocalCacheLocation -EditingEnabled:$true
                }
                else 
                {
                    log "New-OfficeWebAppsFarm -CacheLocation $CacheLocation -CacheSizeInGB $CacheSizeInGB -CertificateName `"$CertificateName`" -InternalURL $InternalURL -ExternalUrl $ExternalURL -LogLocation $LogLocation -LogRetentionInDays $LogRetentionInDays -MaxMemoryCacheSizeInMB $MaxMemoryCacheSizeInMB -RenderingLocalCacheLocation $RenderingLocalCacheLocation -EditingEnabled:true"
                    New-OfficeWebAppsFarm -CacheLocation $CacheLocation -CacheSizeInGB $CacheSizeInGB -CertificateName $CertificateName -InternalURL $InternalURL -ExternalUrl $ExternalURL -LogLocation $LogLocation -LogRetentionInDays $LogRetentionInDays -MaxMemoryCacheSizeInMB $MaxMemoryCacheSizeInMB -RenderingLocalCacheLocation $RenderingLocalCacheLocation -EditingEnabled:$true
                }
