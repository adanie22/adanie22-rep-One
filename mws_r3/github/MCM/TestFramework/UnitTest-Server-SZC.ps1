...
# Desc: Server side Unit testing - SZC  
############################################################################################
...
{
    $url = "http://localhost/"
    $searchString = "ShareFile Storage Server"
    ...
    try
    {
        ...
        $object | Add-Member -Name 'Found?' -MemberType Noteproperty -Value (AddColor $webPageExists "False" "Red")  
...
try
{
    ...
    $shareFilePage = Build-HTML-Fragment (get-ShareFileWebPage) LIST "<h2>ShareFile Storage Server Web Page</h2>"
